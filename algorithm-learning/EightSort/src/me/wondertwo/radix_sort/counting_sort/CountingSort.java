...
public class CountingSort {
    ...
    public static void main(String[] args) {
        ...
        int[] arr = {64, 44, 99, 41, 17, 44, 54, 71, 10, 81, 100, 0, 92, 63, 71, 40, 27};
        print(arr);
        System.out.println("--------------------------------");
    ...
    private static int[] countingSort(int[] arr, int max) {
        ...
        // 循环遍历待排序数组arr，统计每个元素出现的次数，保存在记录数组相应位置
        for (int anArr : arr) {
        ...
        for (int t = arr.length-1; t >= 0; t--) {
            // 由于待排数组下表第一位是0，为了与其下标对应，输出数组OutputTemp[]下表要减1
            // 即：CacheCount[arr[t]]-1
            OutputTemp[CacheCount[arr[t]]-1] = arr[t];
            // 可能待排数组中有重复元素，减1后，方便第二个出现的重复元素放在倒数第二个重复的位置上
            CacheCount[arr[t]] = CacheCount[arr[t]] - 1;
        }
